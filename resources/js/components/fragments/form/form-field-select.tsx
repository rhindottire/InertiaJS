...
import CardContent from '@/components/fragments/card/card';
...
export default function FormFieldSelect<TData>({
  ...
  return (
    <Card className="grid gap-2">
      <CardHeader>
        <CardTitle className="text-xl">{label && <Label className="text-xl">{label}</Label>}</CardTitle>
      </CardHeader>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="p-0">
          <Command>
            <CommandInput defaultValue={displayValue} placeholder={`Search ${label.toLowerCase()}...`} />
            <CommandList>
              ...
              <CommandGroup>
                {data.map((item) => {
                  ...
                  return (
                    ...
                    >
                      {labelText}
                      <Check className={cn('ml-auto h-4 w-4', itemValue === value ? 'opacity-100' : 'opacity-0')} />
                    </CommandItem>
                  );
                })}
              </CommandGroup>
